...
# Forzar codificación UTF-8 para evitar problemas de acentos
...
def main():
    ...
    try:
        ...
        # Rutas estrictas dentro del Storage de Laravel para evitar choques de permisos en Linux
        ...
        # Carga optimizada en CPU consumiendo el mínimo de RAM posible
        whisper_model = os.environ.get("WHISPER_MODEL", "small")
        model = WhisperModel(whisper_model, device="cpu", compute_type="int8", download_root=download_root)
        
        # Transcribir sin arrastrar errores de contexto
        segments, info = model.transcribe(audio_path, language="es", beam_size=5, condition_on_previous_text=False)
